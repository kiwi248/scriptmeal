...
def build_agent():
    ...
    # ── 벡터 DB 로드 (기존 DB 재사용) ───────────────────────
    # 폴더만 있고 실제 DB 파일이 없는 경우도 재생성
    chroma_data_exists = (
        os.path.exists(CHROMA_DIR) and
        os.path.exists(os.path.join(CHROMA_DIR, "chroma.sqlite3"))
    )
    if chroma_data_exists:
        db = Chroma(persist_directory=CHROMA_DIR, embedding_function=embeddings)
    else:
        # DB가 없으면 CSV에서 새로 생성
        df = pd.read_csv(CSV_PATH)
        df_clean = df.dropna(subset=["name", "ingredients", "steps"]).head(2000).copy()
...
[답변]""",
    ...
    # ── RAG 결과 검증 함수 (DB 미검색 시 빈 문자열 반환) ────
    def rag_with_validation(q: str) -> str:
        result = rag_chain.invoke({"query": q})["result"]
        # DB에서 못 찾았다는 표현이 결과에 포함되면 빈 값 반환 → Tavily로 넘어가게
        fail_keywords = ["해당 정보가 데이터에 없", "찾을 수 없", "정보가 없", "데이터베이스에"]
        if any(kw in result for kw in fail_keywords):
            return ""
        return result
    ...
    rag_tool = Tool(
        ...
        description=(
            ...
            "모든 레시피 질문은 반드시 이 툴을 먼저 사용하세요. "
            "결과가 비어있거나 찾지 못한 경우에만 tavily_web_search를 사용하세요."
        ),
        func=rag_with_validation,
    )

    web_search_tool_wrapped = Tool(
        name="tavily_web_search",
        description=(
            "인터넷에서 최신 레시피나 정보를 검색합니다. "
            "recipe_database에서 결과를 찾지 못했을 때만 사용하세요. "
            "최신 트렌드, SNS 유행 레시피 등에도 사용하세요."
        ),
        func=lambda q: "\n".join([
            r.get("content", "") for r in web_search_tool.invoke(q)
        ]) if isinstance(web_search_tool.invoke(q), list) else str(web_search_tool.invoke(q)),
    )
...
[필수 규칙] 레시피/요리 관련 질문은 반드시 아래 순서를 따르세요:
1단계: recipe_database 툴로 Food.com DB를 먼저 검색
2단계: recipe_database 결과가 비어있거나 찾지 못한 경우 → tavily_web_search 툴로 웹 검색
3단계: 두 툴 모두 사용한 후 결과를 종합해서 한국어로 답변

절대로 툴을 사용하지 않고 직접 레시피를 만들어 답변하지 마세요.
인사, 날씨 등 요리와 전혀 무관한 질문만 툴 없이 직접 답변하세요.
반드시 한국어로 답변하세요."""),
    ...
    agent = create_openai_tools_agent(llm, [rag_tool, web_search_tool_wrapped], agent_prompt)
    agent_executor = AgentExecutor(
        agent=agent,
        tools=[rag_tool, web_search_tool_wrapped],
        memory=agent_memory,
        verbose=False,
...
# ── 응답 처리 ──────────────────────────────────────────────
if (send or user_input) and user_input.strip() and engine_ok:
        ...
        try:
            if "에이전트" in mode:
                ...
                tool_label = {
                    ...
                    "tavily_web_search": "🌐 웹 실시간 검색 (Tavily)",
                }.get(tool_used, "🤖 직접 답변")
