...
CHROMA_DIR      = r"C:\0421\chroma_cooking_db"
PROGRESS_FILE   = r"C:\0421\chroma_progress.json"  # 누적 진행상황 저장
...
def build_agent():
    ...
    CHROMA_MAX = 5000   # Chroma 최대 배치 크기 (공식 한도 5461보다 여유있게)
    ...
    def embed_in_batches(db, docs, label="임베딩"):
        ...
        for i in range(0, total, CHROMA_MAX):
            ...
            print(f"[{label}] {inserted}/{total} 청크 완료 ({pct:.1f}%)", flush=True)
    ...
    if os.path.exists(CHROMA_DIR):
        ...
        if remaining:
            embed_in_batches(db, remaining, label="최초 생성")
...
반드시 한국어로 답변하세요."""),
    ...
    agent_executor = AgentExecutor(
        ...
        return_intermediate_steps=True,  # ← 이 줄 추가
    )
        ...
        try:
            if "에이전트" in mode:
                ...
                if len(tools_used) == 0:
                    ...
                    tool_label = tool_label_map.get(tools_used[0], "직접 답변")
                else:
                    # 여러 툴 사용 시 순서대로 표시 (예: Food.com DB 검색 -> 웹 실시간 검색)
