...
Orchestrator Agent
──────────────────
사용자 발화를 분석해 세 가지 의도 중 하나로 분류합니다.

  SPECIFIC_FOOD   – 특정 음식 언급 (떡볶이, 짜장면 등)
  GENERAL_RECIPE  – 조건/상황 기반 레시피 요청 (포만감, 단백질 등)
...
- GENERAL_RECIPE  : 이전 대화의 레시피에 대한 후속 질문 (재료 이유, 대체 재료, 칼로리 질문 등)
...
def classify_intent(client: OpenAI, user_input: str, chat_history: list[dict]) -> str:
    """
    사용자 입력을 분석하고 SPECIFIC_FOOD / GENERAL_RECIPE / OFF_TOPIC 을 반환.
    Function Calling으로 구조화된 응답을 강제해 파싱 실패 위험을 제거.
    """
    #어떤 입력이 들어왔는지, 실제로 함수가 호출됐는지 확인
    print(f"🔍 [Orchestrator] 분류 시작 - 입력: {user_input}")  
    ...
    #히스토리가 제대로 붙어서 가는지 확인
    print(f"🔍 [Orchestrator] 히스토리 포함 메시지 수: {len(messages)}개")
    ...
    #어떤 intent로 분류됐는지 + 왜 그렇게 분류했는지 확인
    print(f"🔍 [Orchestrator] 분류 완료 - intent: {intent}, reasoning: {args.get('reasoning')}")
