...
SEARCH_TOOL = [
    {
        ...
        "function": {
            "name": "search_recipe",
            "description": "모르는 음식이나 최신 유행 음식 레시피를 검색할 때 사용. query는 반드시 '음식명 레시피 만드는법 재료'처럼 레시피 검색에 최적화된 형태로 작성할 것",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "레시피 검색 쿼리. 예: '두바이쫀득쿠키 레시피 만드는법 재료'"}
                },
                "required": ["query"],
            },
        },
...
{products_text}"""
        ...
        # 7단계: 최종 응답 포맷
        result = self._build_response(recipe["recipe_text"], product_details)
    ...
    def _handle_tool_call(self, first_response, user_query, chat_history, products_text):
        ...
        print(f"🍖 [SpecificFood] Tavily 검색 쿼리: {search_query}")
        ...
        for r in results:
            print(f"🍖 [SpecificFood] Tavily URL: {r.get('url', '')} / raw_content 길이: {len(r.get('raw_content', '') or '')}")
        ...
        # )[:3000]
       # 첫 번째 결과에서 15000자
        search_text = (results[0].get("raw_content", "") or results[0].get("content", ""))[:15000] if results else ""
